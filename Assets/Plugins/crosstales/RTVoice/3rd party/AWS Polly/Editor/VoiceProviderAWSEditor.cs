...
namespace Crosstales.RTVoice.AWSPolly
{
   ...
   public class VoiceProviderAWSEditor : Editor
   {
      ...
      public override void OnInspectorGUI()
      {
         ...
         GUI.enabled = !(script.UseNeuralVoices && script.hasNeuralVoices);
         autoBreath = EditorGUILayout.Toggle(new GUIContent("Auto Breath", "Enables or disables the simulation of natural breathing while speaking (default: true). Ignored if neural voices are used."), script.AutoBreath);
         ...
         useNeural = EditorGUILayout.Toggle(new GUIContent("Use Neural Voices", "Enable or disable neural voices (default: false). Note: SSML is not supported for neural voices."), script.UseNeuralVoices);
