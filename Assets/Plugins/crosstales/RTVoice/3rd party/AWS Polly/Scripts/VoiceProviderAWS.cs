...
namespace Crosstales.RTVoice.AWSPolly
{
   ...
   public class VoiceProviderAWS : Provider.BaseCustomVoiceProvider
   {
      ...
      [UnityEngine.Serialization.FormerlySerializedAsAttribute("AutoBreath")] [Header("Voice Settings"), Tooltip("Enables or disables the simulation of natural breathing while speaking (default: true). Ignored if neural voices are used."), SerializeField]
      ...
      [UnityEngine.Serialization.FormerlySerializedAsAttribute("UseNeuralVoices")] [Tooltip("Enable or disable neural voices (default: false). Note: SSML is not supported for neural voices."), SerializeField]
...
//#endif
      ...
      /// <summary>Enables or disables the simulation of natural breathing while speaking. Ignored if neural voices are used.</summary>
...
//#endif
      ...
      private void load()
      {
         if (cachedVoices?.Count == 0)
         {
            ...
            neuralVoices.Add(new Model.Voice("Kevin", "English, US (en-US)", Model.Enum.Gender.MALE, "Child", "en-US")); //neural only
            neuralVoices.Add(new Model.Voice("Olivia", "English (Australian) (en-AU)", Model.Enum.Gender.FEMALE, "Adult", "en-AU")); //neural only
            neuralVoices.Add(new Model.Voice("Gabrielle", "French (Canadian) (fr-CA)", Model.Enum.Gender.FEMALE, "Adult", "fr-CA")); //neural only
               ...
               else
               {
                  ...
                  else
                  {
                     ...
                     Amazon.Polly.Model.SynthesizeSpeechRequest synthesizeSpeechPresignRequest = new Amazon.Polly.Model.SynthesizeSpeechRequest
                     {
                        ...
                        TextType = wrapper.ForceSSML && !(useNeuralVoices && hasNeuralVoices) ? Amazon.Polly.TextType.Ssml : Amazon.Polly.TextType.Text,
                        VoiceId = voiceName,
...
*/
      private string prepareText(Model.Wrapper wrapper, string voiceCulture)
      {
         ...
         wrapper.ForceSSML = !(useNeuralVoices && hasNeuralVoices); //enable auto-clearing of all SSML-tags when neural voices are enabled
